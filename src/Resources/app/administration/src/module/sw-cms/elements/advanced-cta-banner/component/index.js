...
Component.register("sw-cms-el-advanced-cta-banner", {
  template,
  ...
  computed: {
    category() {
      if (
        !this.element.data ||
        !this.element.data.category ||
        !this.element.data.category.media
      ) {
    ...
    product() {
      if (
        ...
        !this.element.data.product.cover
      ) {
    ...
    backgroundCss() {
      if (!this.element.config.videoActive.value) {
        return {
          ...
          "background-attachment": this.element.config.backgroundFixed.value
            ? "fixed"
            : "initial",
          ...
          "background-size":
            this.element.config.backgroundDisplayMode.value === "custom"
              ? `${this.element.config.backgroundSizeX.value} ${this.element.config.backgroundSizeY.value}`
              : this.element.config.backgroundDisplayMode.value,
        };
      }
    },
    ...
    boxCss() {
      return {
        display:
          this.element.config.iconPosition.value === "top" ? "block" : "flex",
  ...
  methods: {
    createdComponent() {
      this.initElementConfig("advanced-cta-banner");
      this.initElementData("advanced-cta-banner");
    },
    ...
    async setMedia() {
      if (this.element.config.iconMedia.value) {
        const mediaEntity = await this.mediaRepository.get(
          this.element.config.iconMedia.value,
        ...
        this.$set(
          this.element.data,
          "iconMediaId",
          mediaEntity === null ? null : mediaEntity.id,
        );
        this.$set(this.element.data, "iconMedia", mediaEntity);
      }

      if (this.element.config.media.value) {
        const mediaEntity2 = await this.mediaRepository.get(
        ...
        this.$set(
          this.element.data,
          "mediaId",
          mediaEntity2 === null ? null : mediaEntity2.id,
        );
